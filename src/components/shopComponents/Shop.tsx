...
const Shop:React.FC<Prop> = ({ value, delButton , heartButton }) => {
    const {currnetUser, addToWishlist, addToCart, deleteFromWishlist } = useUser();
            ...
            <div key={index} className='w-full border h-[350px] relative group'>
                ...
                <div className='px-2'>
                    <div className='text-[10px] md:text-[12px] py-2'>
                        <span className='border rounded-full px-2 py-1'>{product.gender}</span>
                        <span className='border rounded-full mx-2 px-2 py-1'>{product.category}</span>
                    </div>
                    <p className={`text-[10px] py-1 md:py-0 md:text-[12px] text-red-500 ${isProductQuantityLow(product.quantity) ? '' : 'hidden'}`}>Only few left</p>
                    <p className='text-wrap font-semibold text-sm md:text-base'>{product.productName}</p>
                    <p className='text-[12px] md:text-sm text-gray-500'>{product.productDesc}</p>
                    <div className='flex gap-2 text-sm md:text-base'>
                        <p className='text-gray-500 line-through'>₹{product.price}</p>
                        <p className='font-semibold'>₹{discountedPrice(product.price, product.discountPercent)}</p>
                        <p className='text-orange-400'>{product.discountPercent}% OFF</p>
                    </div>
                </div>

                { /* Hover Face */ }

                <button  onClick={()=>router.push(`/product/${product._id}`)}  className='w-full h-full absolute top-0 flex items-center justify-center flex-col custom-transition opacity-0 group-hover:opacity-100 bg-[rgba(0,0,0,0.4)] backdrop-blur-sm'>
